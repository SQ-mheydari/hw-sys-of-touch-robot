...
from optocamera import OptoCamera, CameraType, DeviceBusy, OptoCameraFrame, CaptureError
from typing import Optional, Union, Tuple, Callable, List
from threading import Event
from queue import Queue
from queue import Empty
from pypylon import genicam
# For type checking only. Python 3.5 does not support TYPE_CHECKING only imports
import numpy as np
...
class Enumerator:
    """Base class for Pylon enumerator proxies. """
    @classmethod
    def values(cls):
        """Lists all supported values in this enumerator. """
        return [k for k in cls.__dict__
                if not k.startswith('_') and isinstance(cls.__dict__[k], str)]


class AcquireMode(Enumerator):
    """
    Support yasler capture modes
    """
    Continuous = 'Continuous'
    SWTriggered = 'SWTriggered'
    HWTriggered = 'HWTriggered'
    HWTriggeredBurst = 'HWTriggeredBurst'


class Camera_Optocamera_Basler(Camera):
    """
    Class to control camera through optocamera-basler. Tries to imitate yasler driver api in order to be
    easy replacement for yasler.
    """
    def __init__(self,
        ...
        if 'binning_horizontal_mode' in kwargs:
            self.binning_horizontal_mode = kwargs['binning_horizontal_mode']

        if 'binning_vertical_mode' in kwargs:
            self.binning_vertical_mode = kwargs['binning_vertical_mode']

        # set pixel format if given
        if 'pixel_format' in kwargs:
            self.pixel_format = kwargs['pixel_format']

        # set callback to call during continuous capture
    ...
    # Amount of hand-defined properties might seem a bit excessive but it is the least painful method to support
    # the set and get parameters functions since optocamera and tnt-gt camera apis use slightly different names for
    # some parameters.
    @property
    ...
    @property
    def binning_horizontal_mode(self) -> str:
        return self._get_enum_symbolic('BinningModeHorizontal')

    @binning_horizontal_mode.setter
    def binning_horizontal_mode(self, value: str):
        self._set_enum_symbolic('BinningModeHorizontal', value)

    @property
    def binning_vertical_mode(self) -> str:
        return self._get_enum_symbolic('BinningModeVertical')

    @binning_vertical_mode.setter
    def binning_vertical_mode(self, value: str):
        self._set_enum_symbolic('BinningModeVertical', value)

    @property
    def trigger_selector(self) -> str:
        return self._get_enum_symbolic('TriggerSelector')

    @trigger_selector.setter
    def trigger_selector(self, value: str):
        self._set_enum_symbolic('TriggerSelector', value)
    ...
    def trigger_source(self, value: str):
        self._set_enum_symbolic('TriggerSource', value)

    @property
    def trigger_mode(self) -> str:
        return self._get_enum_symbolic('TriggerMode')

    @trigger_mode.setter
    def trigger_mode(self, value: str):
        self._set_enum_symbolic('TriggerMode', value)

    @property
    def acquisition_mode(self) -> str:
        return self._get_enum_symbolic('AcquisitionMode')

    @acquisition_mode.setter
    def acquisition_mode(self, value: str):
        self._set_enum_symbolic('AcquisitionMode', value)

    @property
    def burst_frame_count(self) -> int:
        """
        "The amount of frames to be grabbed before the camera goes "
                   "back to waiting for acquisition start command."
        Actually uses AcquisitionFrameCount genicam parameter to be compatible with old yasler driver
        """
        if self.camtype == 'gige':
            return self._camera.get_int_parameter('AcquisitionFrameCount')
        elif self.camtype == 'usb':
            return self._camera.get_int_parameter('AcquisitionBurstFrameCount')

    @burst_frame_count.setter
    def burst_frame_count(self, value: int):
        if self.camtype == 'gige':
            self._camera.set_int_parameter('AcquisitionFrameCount', value)
        elif self.camtype == 'usb':
            self._camera.set_int_parameter('AcquisitionBurstFrameCount', value)
    ...
    def _default_callback(self, cam: OptoCamera, frame: OptoCameraFrame, error: CaptureError) -> bool:
        """
    ...
    def start_continuous(self, callback: Optional[Callable[[np.ndarray, Exception], None]] = None,
        ...
        if trigger_type == 'SW':
            self._set_mode(AcquireMode.Continuous)
        elif trigger_type == 'HW':
            self._set_mode(AcquireMode.HWTriggered)
        elif trigger_type == 'HWBurst':
            self._set_mode(AcquireMode.HWTriggeredBurst)
        else:
            raise ValueError("trigger_type: {} must be one of following: 'SW', 'HW', 'HWBurst'".format(trigger_type))
    ...
    def burst(self, n: int = 1, trigger_type: str = 'SW', timeout: Optional[float] = None) -> List[np.ndarray]:
        """
        Take a set of images with given trigger settings. Uses continuous capture to acquire images
        :param n: how many images to take
        :param trigger_type: Trigger type to use (SW, HW or HWBurst)
        :param timeout: timeout for the burst sequence operation
        """
        if trigger_type == 'HW':
            self._set_mode(AcquireMode.HWTriggered)
        elif trigger_type == 'HWBurst':
        ...
        def burst_callback(cam, frame, error):
            """
            Callback for OptoCamera image acquisition
            """
            images.append(Camera_Optocamera_Basler.frame_to_array(out_format, frame))
            if len(images) >= n:
                burst_ready.set()
                # Returning false stops the acquisition
                return False
            # Continue
            return True

        self._camera.start_acquisition(burst_callback)
        if not burst_ready.wait(timeout=timeout):
            # Stop acquisition in case of timeout
            self._camera.stop_acquisition()
            raise TimeoutError('Burst image acquisition timed out')
        return images

    def set_burst_frame_count(self, amount: int):
        """
        Set the amount of HW trigger signals allowed before the acquisition needs to be restarted.
        """
        self.burst_frame_count = amount

    @staticmethod
    def frame_to_array(out_format: str, frame: OptoCameraFrame):
        """
    ...
    def _capture_image(self):
        ...
        return Camera_Optocamera_Basler.frame_to_array(self._format, frame)

    def _get_enum_symbolic(self, name: str) -> str:
        """
        OptoCamera-basler currently (1.0) only support setting Enums using integer values. This method imitates
        optocamera baslers own get and set enum value methods but uses GetValue and SetValue instead of
        GetIntValue and SetIntValue. Using OptoCamera-baslers internal methods is necessary to avoid copy-pasting
        a lot of code from OptoCamera Basler. If OptoCamera-baslers genicam/enum support is improved, this solution
        could be altered to avoid using private methods.
        """
        #
        node = self._camera._get_type_node(genicam.IEnumeration, name)
        return node.GetValue()

    def _set_enum_symbolic(self, name: str, value: str):
        node = self._camera._get_type_node(genicam.IEnumeration, name)
        return node.SetValue(value)

    def _set_mode(self, mode: str):
        """Shortcut for setting the mode. Copied with modifications from yasler"""
        if mode == AcquireMode.SWTriggered:
            # turn off possible burst capture trigger to make sure only one trigger is 'On'
            if self.camtype == 'usb':
                self.trigger_selector = 'FrameBurstStart'  # USB camera
            else:
                self.trigger_selector = 'AcquisitionStart'  # GigE camera
            self.trigger_mode = 'Off'

            # change selector to single frame capture and apply settings
            self.trigger_selector = 'FrameStart'
            self.trigger_source = 'Software'
            self.trigger_mode = 'On'
            self.acquisition_mode = 'Continuous'
        elif mode == AcquireMode.Continuous:
            # turn off possible burst capture trigger
            if self.camtype == 'usb':
                self.trigger_selector = 'FrameBurstStart'  # USB camera
            else:
                self.trigger_selector = 'AcquisitionStart'  # GigE camera
            self.trigger_mode = 'Off'
            # also turn of single frame capture
            self.trigger_selector = 'FrameStart'
            self.trigger_mode = 'Off'
            self.acquisition_mode = 'Continuous'
        elif mode == AcquireMode.HWTriggered:
            # turn off possible burst capture trigger to make sure only one trigger is 'On'
            if self.camtype == 'usb':
                self.trigger_selector = 'FrameBurstStart'  # USB camera
            else:
            ...
            # change selector to single frame capture and apply settings
            self.trigger_selector = 'FrameStart'
            self.trigger_source = 'Line1'
            self.trigger_mode = 'On'
            self.acquisition_mode = 'Continuous'
        elif mode == AcquireMode.HWTriggeredBurst:
            # single frame start trigger must be turned off for the burst trigger to work
            self.trigger_selector = 'FrameStart'
            self.trigger_mode = 'Off'

            # change selector to burst mode capture and apply settings
            if self.camtype == 'usb':
                self.trigger_selector = 'FrameBurstStart'  # USB camera
            else:
                self.trigger_selector = 'AcquisitionStart'  # GigE camera
