...
# Set environment path to locate DLLs in development environment and use only DLL names to load librarys.
# DLLs are copied to run directory in pyinstalled version.
...
if sys.maxsize > 2**32:
    ...
    c_arr_ops = CDLL('xiArrOps64.dll')

    #library for communication with device
    _device = CDLL('xiapi64.dll')
    ...
    #library for operations with image data - to increase speed with which
    #get_image_data_numpy() returns data
    c_arr_ops = CDLL('xiArrOps32.dll')

    #library for communication with device
    _device = CDLL('xiapi32.dll')
