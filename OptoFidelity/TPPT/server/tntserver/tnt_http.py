...
from tntserver.Nodes.Node import NodeException, Node
...
class RequestHandler(http.server.BaseHTTPRequestHandler):
        ...
        try:
            if not hasattr(handler, 'decodes'):
                ...
                if not isinstance(out, tuple):
                    raise NodeException("Internal callback did not return tuple")
            else:
    ...
    def respond(self, parts, scheme):
        ...
        # Traverse tree with parts
        #
        log.info("Handling HTTP %s for %s", scheme.upper(), "/".join(parts))
...
class TntServerThreadingServer(socketserver.ThreadingMixIn, socketserver.TCPServer):
#class TntMiniThreadingServer(socketserver.TCPServer):
